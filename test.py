...
def execute():
    ...
    train_logits_var = Variable(train_logits)
    train_probabilities = softmax(train_logits_var).data
    ...
    val_logits_var = Variable(val_logits)
    val_probabilities = softmax(val_logits_var).data
    ...
    test_logits_var = Variable(test_logits)
    test_probabilities = softmax(test_logits_var).data
    ...
    train_entropy = compute_entropy(train_logits_var, dim=1)
    train_mean_entropy = train_entropy.data.sum()/train_entropy.data.size(0)
    test_entropy = compute_entropy(test_logits_var, dim=1)
    ...
    openmax_test_probabilities = compute_openmax(
        test_logits_var.data.numpy(), weibull_models, distance_type)
    ...
    return (map_val_acc1, train_mean_entropy, test_mean_entropy,
...
def relative_entropy_kernel(tensor1, tensor2):
    ...
    tensor1_var = Variable(tensor1)
    tensor2_var = Variable(tensor2)
    tensor1_probabilities = softmax(tensor1_var).data
    tensor2_probabilities = softmax(tensor2_var).data
...
def compute_entropy(variable, dim=1):
    ...
    softmax_output = local_softmax(variable)  # .data)#.data
...
def main():
    ...
    for experiment in args.experiments:
        ...
        for execution in range(1, args.executions + 1):
            ...
            # execute experiment...
            (map_val_acc1, train_mean_entropy, test_mean_entropy, threshold_auc,
             openmax_auc, isoforest_auc, ocsvm_auc) = execute()

            # results and statistics...
            (auc_results["MAP VAL [ACC1]"], auc_results["TRAIN MEAN ENTROPY"], auc_results["TEST MEAN ENTROPY"],
             auc_results["THRESHOLD [AUC]"], auc_results["OPENMAX [AUC]"],
             auc_results["ISOFOREST [AUC]"], auc_results["OCSVM [AUC]"]) =\
                (100 * map_val_acc1, train_mean_entropy, test_mean_entropy,
                 threshold_auc, openmax_auc, isoforest_auc, ocsvm_auc)

            # appending results...
            auc_stats = auc_stats.append(auc_results, ignore_index=True)
            auc_stats = auc_stats[["MAP VAL [ACC1]", "TRAIN MEAN ENTROPY", "TEST MEAN ENTROPY",
        ...
        all_auc_stats[experiment] = auc_stats

    print("\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n", "OVERALL STATISTICS", "\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
    for key in all_auc_stats:
        print("\n", key.upper())
        # print("\n", all_auc_stats[key].transpose())
        # print("\n", all_auc_stats[key].describe())
        print("\n", all_auc_stats[key].describe().loc[['mean','std']])
